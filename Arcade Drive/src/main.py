...
Elevation = DigitalOut(brain.three_wire_port.e)
...
def drive():
    global timer, time_alert, allowElevation
    allowElevation = True 
...
def auton():
    ...
    if selector == 0: # Near side with hanging pole touch
        ...
        top_arm_joint.spin_for(REVERSE, 850, DEGREES)  
        fly_wheel.set_velocity(100, PERCENT)      
        fly_wheel.spin(REVERSE)
        wait(30, SECONDS)
        fly_wheel.stop()
        # top_arm_joint.set_stopping(COAST)
        # top_arm_joint.spin_to_position(5, DEGREES, False)
        # bottom_arm_joint.spin_to_position(5, DEGREES, False)
        # drivetrain.set_turn_velocity(35, PERCENT)
        # drivetrain.set_drive_velocity(100, PERCENT)
...
def elevate():
    global allowElevation, e
    if allowElevation and e == 0:
        Elevation.set(True)
        e = 1
    elif allowElevation and e == 1:
        Elevation.set(True)
        e = 0

def goodfunctionname():
    top_arm_joint.set_stopping(COAST)
    bottom_arm_joint.set_stopping(COAST)
    bottom_arm_joint.spin_for(FORWARD, 3, DEGREES)
    top_arm_joint.spin_for(FORWARD, 3, DEGREES)
    bottom_arm_joint.spin_for(REVERSE, 3, DEGREES)
    top_arm_joint.spin_for(REVERSE, 3, DEGREES)
    top_arm_joint.set_stopping(HOLD)
    bottom_arm_joint.set_stopping(HOLD)
    wait(20, MSEC)
